...
from config import (DESKTOP_PATH, ORGANIZER_UNDO_LOG,
                    ORGANIZER_CONFIRM_TIMEOUT)
...
def _build_preview(categorized: dict) -> str:
    ...
    return (f"I found {total} {'file' if total == 1 else 'files'} on your Desktop. "
            f"{summary}. "
...
def preview_organize(folder_path: str = None) -> str:
    ...
    try:
        ...
        if not categorized:
            return "Your Desktop is already clean. No files to organize."
        ...
        return _build_preview(categorized)
...
def confirm_organize() -> str:
    ...
    try:
        ...
        result = f"Done. Moved {moved_count} files into {len(categorized)} folders on your Desktop."
        if failed_count:
            result += f" {failed_count} files could not be moved."
        result += " I've saved an undo log in case you want to reverse this."
        ...
        for item in folder.iterdir():
            if item.is_dir():
                try:
                    ...
                    pass           # not empty, skip
        if removed:
            return f"Removed {removed} empty folders from Desktop."
