...
def _fetch_internshala(role: str) -> list:
    ...
    url = f"https://internshala.com/fresher-jobs/{role_slug}-jobs-in-south-india/"
    ...
    try:
        ...
        cards = soup.select(".individual_internship")[:10]
        ...
        for card in cards:
            try:
                ...
                loc     = _clean_text(loc_el.text) if loc_el else "South India"
...
# ── Session Tools (called from intent router in JOB_SEARCH mode) ──────────────
...
def enter_job_search_mode(role: str = "all") -> str:
    ...
    speak_intro = f"Entering job search mode. Searching for {role_label} fresher roles in South India. One moment."
...
def next_jobs() -> str:
    ...
    if idx >= len(_search_session["results"]):
        _search_session["current_index"] = 0
        return "That's all the jobs I found. Starting from the beginning. " + \
...
def open_job(number: int) -> str:
    """Open a specific job by number in browser."""
    if not _search_session["active"]:
        return "No active job search."

    jobs = _search_session["results"]
    idx  = _search_session["current_index"] + number - 1
...
def save_current_job(number: int = 1) -> str:
...
def exit_job_search() -> str:
    ...
    if saved:
        return (f"Exiting job search mode. You saved {saved} jobs out of {total} found. "
                f"Check saved underscore jobs dot csv in your job results folder.")
